...
# varaible is used to search the users table for a user and the password
...
def change_password(username, password):
	...
	if nullcheck == None:
		...
		crs.execute("UPDATE users SET password = (?) WHERE username = (?)", (password, username))
...
def add_record(name, account_number, amount):
	# Connect to database
	cnct = sqlite3.connect('data.db')
	# Create a cursor
	crs = cnct.cursor()

	crs.execute("INSERT INTO accounts (name, account, amount) VALUES (?,?,?)", (name, account_number, amount))
...
# search_database function that accepts one variable that is used to search the 
# accounts table for an account number matching the variable.
def search_database(account_number):
	# Connect to database
	cnct = sqlite3.connect('data.db')
	# Create a cursor
	crs = cnct.cursor()

	crs.execute("SELECT * FROM accounts WHERE account = (?)", (account_number,))
	nullcheck = crs.fetchone()
...
# update_amount function that accepts two variables that are used to update
# the amount of funds for the account passed into this function. 
def update_amount(account_number, amount):
	# Connect to database
	cnct = sqlite3.connect('data.db')
	# Create a cursor
	crs = cnct.cursor()
	...
	if nullcheck == None:
		...
		crs.execute("UPDATE accounts SET amount = (?) WHERE account = (?)", (amount, account_number))
...
def delete_record(account_number):
	...
	if nullcheck == None:
		...
		crs.execute("DELETE FROM accounts WHERE account = (?)", (account_number,))
		printString = "Account " + str(account_number) + " deleted successfully..."
		print(printString)
		print("")
